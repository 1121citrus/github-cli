#!/usr/bin/env bats
# test/06-image-metadata.bats — verify OCI image labels and build-arg wiring.
#
...
@test "Dockerfile has LABEL org.opencontainers.image.documentation" {
    run grep -F 'org.opencontainers.image.documentation' "${DOCKERFILE}"
    [ "$status" -eq 0 ]
}
...
@test "Dockerfile has LABEL org.opencontainers.image.vendor" {
    run grep -F 'org.opencontainers.image.vendor' "${DOCKERFILE}"
    [ "$status" -eq 0 ]
}
...
@test "Dockerfile has LABEL org.opencontainers.image.authors" {
    run grep -F 'org.opencontainers.image.authors' "${DOCKERFILE}"
    [ "$status" -eq 0 ]
}
